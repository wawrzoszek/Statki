...
public class Ship {
	...
	public void wpiszStatek(Plansza p, int rozmiarStatku) 
	{
		...
		if (poprawny_kierunek == true) 
		{
			...
			{
				if (kierunek == 0) {os[0] = os[0] - 1;}
				if (kierunek == 1) {os[0] = os[0] + 1;}
				if (kierunek == 2) {os[1] = os[1] - 1;}
				if (kierunek == 3) {os[1] = os[1] + 1;}
			...
			//jesli na kt—rymæ z miejsc nowego statku jest juz statek wr— do poczˆtku
			...
			for (int j = 0; j < rozmiarStatku; j++) {
				...
				try 
				{
					...
					System.out.println("x"+elementy_statku_x[j]+" y"+elementy_statku_y[j]+" pk: "+pk[elementy_statku_x[j]-1][elementy_statku_y[j]]);
			...
			//jeæli jest wolne miesce to zapisz w kaýdym punkcie nowego statku jego cyfr«
